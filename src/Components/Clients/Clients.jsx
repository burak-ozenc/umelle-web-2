...
// Data
import {TechList} from "./ClientsData";
        ...
        case "client-logo-style-06":
            return (
                <Col key={i} className={params.className}>
                    <a target={item.target} className="inline-block" aria-label="clientlogo"
                       href={item.link ? item.link : "#"}>
                        <m.div {...{...params.animation, transition: {delay: i / 4, duration: 0.8}}}>
                            {item.img &&
                                <img width="148" height="43" className="w-[auto]" src={item.img} alt="clientlogo"/>}
                        </m.div>
                    </a>
                </Col>
            )
            ...
            return (
                <Col key={i} className={params.className}>
                    <m.div
                        ...
                        ...params.animation,
                        transition: {delay: i / 4, duration: 0.8}
                    }}>
...
const Clients = (props) => {
    ...
    return (
        props.carousel === true ? (
            <m.div
                className={`client-slider-wrapper relative${props.className ? ` ${props.className}` : ""}`} {...props.animation}>
                <Swiper
                    ref={swiperRef}
                    slidesPerView={4}
                    spaceBetween={20}
                    ...
                    props.carouselOption.navigation && (
                        <>
                            <div onClick={() => swiperRef.current.swiper.slidePrev()}
                                 className="swiper-button-prev absolute"></div>
                            <div onClick={() => swiperRef.current.swiper.slideNext()}
                                 className="swiper-button-next absolute"></div>
                        </>
                    )
                }
            </m.div>
