...
// Data
import HeaderData from "./HeaderDataAdjusted";
...
export const Menu = memo((props) => {
    ...
    return (
        <div
            className={`${props.mobileMenu ? `mobile-menu-${props.mobileMenu}` : ""}${props.className ? ` ${props.className}` : ""}`}>
            <ul className="navbar-nav">
                {props.data.map((item, i) => {
                    return (
                        <li className={`nav-item${item.dropdown || item.megamenu ? ` dropdown` : ""}${isMenuActive === i ? " open" : ""}`}
                            key={i}>
                            {
                                item.link ? (
                                                            ...
                                                            item.dropdown.map((item, i) => {
                                                                return (
                                                                    <li className="nav-item" key={i}>
                                                                        ...
                                                                        {(item.img && item.link) &&
                                                                            <Link to={item.link}><img height="235"
                                                                                                      alt="menu-banner"
                                                                                                      width="210"
...
export const MobileMenu = (props) => {
    ...
    return (
        <div className={props.className ? ` ${props.className}` : ""}>
            <Navbar.Toggle id="toggle-bar" className="order-last md:px-[25px] sm:ml-0"
                                            ...
                                            <div className="megamenu" ref={megamenu_ref}>
                                                <div className="flex">
                                                    {item.megamenu.map((item, i) => {
                                                        return (
                                                            <ul key={i}
                                                                className={`${(item.dropdown.filter(item => item.img).length > 0) ? "img-wrapper" : "inline-block"}`}>
        ...
        <>
            {[false].map((expand) => (
                ...
                >
                    ...
                    <div
                        className={`${show ? "block h-[100vh] left-0 overflow-hidden fixed top-0 w-full z-[999]" : ""}`}>
                        <div
                            ref={ref} className={`hamburger-menu-wrapper pos-${props.position}${show ? " show" : ""
                        }${props.className ? ` ${props.className}` : ""}`}
                        >
                            {props.closeBtn && (
                                <button aria-label="hamburger menu close button" className="close-btn"
...
export const SearchBar = memo((props) => {
    ...
    return (
        <div
            className={`header-search-iconbar inline-block align-middle pl-[17px] text-[17px] leading-none${props.className ? ` ${props.className}` : ""}`}
            style={props.style}>
            <Link to="#" aria-label="search" className="search-form-icon leading-[20px]"
                  onClick={(e) => e.preventDefault()}>
                <i className={`feather-search px-0 inline-block${props.className ? ` ${props.className}` : ""}`}
                   onClick={() => setSearchModalOpen(true)}></i>
            </Link>
                            ...
                            >
                                <div className="search-form-box">
                                    <Input
                                        ...
                                        label={
                                            <span
                                                className="search-label font-medium text-spanishgray text-sm font-serif uppercase block">
...
export const HeaderLanguage = (props) => {
    return (
        <div
            className={`header-language dropdown inline-block align-middle pl-[17px] text-[17px]${props.className ? ` ${props.className}` : ""}`}
            style={props.style}>
...
export const HeaderCart = (props) => {
    return (
        <div
            className={`header-cart-icon dropdown mr-[10px] inline-block align-middle pl-[17px] text-[17px]${props.className ? ` ${props.className}` : ""}`}
            style={props.style}>
            <Link aria-label="link" to="#" className="relative inline-block">
                ...
                <span
                    className="cart-count font-serif bg-basecolor text-white absolute top-[20px] right-[-10px] w-[16px] h-[16px] text-center text-[9px] leading-[16px] rounded-full">
          2
        </span>
            </Link>
                ...
                <li className="cart-item items-center">
                    ...
                    <div className="product-image">
                        <Link aria-label="link" to="#">
                            <img src="https://via.placeholder.com/150x191" className="cart-thumb" alt="cart-product"
                                 width={50} height={67}/>
                        </Link>
                    </div>
                                                            ...
                                                            <Accordion.Header>
                                                                {
                                                                    item.link ? (
                                                                            <Link aria-label="link" className="menu-link"
                                                                                  to={item.link}> {item.title} </Link>)
                                                                        : (<span
                                                                            className="menu-link"> {item.title} </span>)
                                                                }
                                                                {item.dropdown && (
                                                                    <span className="icon"></span>
                                                                )}
                                        ...
                                        <div className="megamenu">
                                            <Accordion>
                                                {item.megamenu.map((item, i) => {
                                                    return (
                                                        <Accordion.Item key={i} eventKey={i}
                                                                        className={`${(item.dropdown.filter(item => item.img).length > 0) ? "img-wrapper" : ""}`}>
                                                            <Accordion.Header>
                                                                <span className="menu-link">{item.title}</span>
                                                                {item.dropdown && (
                                                                    <span className="icon"></span>
